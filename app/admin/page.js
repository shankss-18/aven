...
export default function AdminOrdersPage() {
  ...
  useEffect(() => {
    // Redirect the /admin root to analytics (the new default landing)
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-[#e4e0d2] pb-5">
        <div>
          ...
          <p className="text-xs text-[#8f8a7a] mt-1">
            Monitor incoming customer purchases, review payment status, and dispatch shipments.
          </p>
        </div>
        <div className="flex items-center gap-2 self-start sm:self-auto">
          <span className="font-mono text-xs text-[#5a5744] bg-white px-3 py-1.5 rounded-lg border border-[#e4e0d2] shadow-xs">
            Total Orders: <strong className="text-[#0e0e0c]">{orders.length}</strong>
          </span>
        </div>
      </div>
      ...
      <div className="bg-white border border-[#e4e0d2] rounded-xl overflow-hidden shadow-xs">
        {orders.length === 0 ? (
          <div className="py-16 text-center text-[#8f8a7a] space-y-2">
            <svg
              className="w-10 h-10 mx-auto stroke-current fill-none stroke-[1.2] text-[#c9c4b3]"
              viewBox="0 0 24 24"
            >
            ...
            <p className="text-sm font-medium text-[#0e0e0c]">No orders found</p>
            <p className="text-xs text-[#8f8a7a]">
              Customer orders will appear here once checkout transactions are completed.
            </p>
          </div>
        ) : (
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-[#f2efe6] text-sm">
                {orders.map((order) => (
                  ...
                  >
                    <td className="px-5 py-4 font-mono font-medium text-[#0e0e0c]">
                      #{order.id}
                    </td>
                    <td className="px-5 py-4">
                      ...
                      <div className="text-xs font-mono text-[#8f8a7a]">
